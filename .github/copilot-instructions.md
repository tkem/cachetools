...
### Decorators
- `@cached` (`_cached.py`): Function memoization; separate wrappers for each lock/condition/info combination. `cache=None` was deprecated in 7.2 and **removed in 8.0**: `cached()` now rejects it eagerly with `TypeError("cache must not be None")` before returning the decorator, and the `_uncached`/`_uncached_info` wrappers are gone. Only `None` is checked — other non-mappings still fail late; a `# TODO` in `cached()` tracks tightening this to `collections.abc.Mapping`
- `cached()` builds the `info=True` `make_info` closure itself: `cache.maxsize`/`cache.currsize` for a `Cache`, `(None, len(cache))` for any other `collections.abc.Mapping`, and `(0, 0)` otherwise — the last branch is reachable only via a duck-typed non-`Mapping` cache, covered by `NonMappingWrapperTest`
- `@cachedmethod` (`_cachedmethod.py`): Method memoization; `_WrapperBase` (per-instance callable, holds `__wrapped__`/cache/key/lock/cond) is subclassed by six explicit wrapper classes — `_UnlockedWrapper`, `_LockedWrapper`, `_ConditionWrapper`, and their `*Info` variants — one per lock/condition/info combination, selected by the `_wrapper()` factory (previously generated dynamically via nested per-combo classes); each `_Condition*Wrapper` keeps a per-instance pending set; `cache(self)` must return a cache object, not `None`
...
- `InvalidCacheTest` in `test_cached.py` asserts that both `cached(None)` and `cached(None, info=True)` raise `TypeError` at *decoration* time — the returned decorator is deliberately never applied — note `cachedmethod` has no equivalent eager check, so a `cache(self)` returning `None` still fails late with `TypeError` (`NoneMethodTest`)
- `NonMappingWrapperTest` in `test_cached.py` supplies a duck-typed cache that is not a `collections.abc.Mapping`, covering the `(0, 0)` `make_info` branch
